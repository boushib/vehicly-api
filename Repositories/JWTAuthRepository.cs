...
namespace vehiclesStoreAPI.Repositories
{
  public class JWTAuthRepository : IJWTAuthRepository
  {
    ...
    public IImmutableDictionary<string, RefreshToken> UserRefreshTokenDictionary => _userRefreshTokens.ToImmutableDictionary();
    ...
    {
      var shouldAddAudienceClaim = string.IsNullOrEmpty(claims?.FirstOrDefault(c => c.Type == JwtRegisteredClaimNames.Aud)?.Value);
    ...
    public void RemoveExpiredRefreshTokens(DateTime now)
    {
      ...
      {
        _userRefreshTokens.TryRemove(token.Key, out _);
      }
    }
